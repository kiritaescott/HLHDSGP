    ...
    private BaseSchedulingAlgorithm getScheduler(SchedulingAlgorithm name) {
        ...
        switch (name) {
            ...
            case HEFT:
                ...
                break;
            case GRPHEFT:
                algorithm = new GRPHeftSchedulingAlgorithm();
                break;
            case GP:
                algorithm = new GPSchedulingAlgorithm();
                break;
            case BCDSGP:
                algorithm = new BCDSGPSchedulingAlgorithm();
                break;
            case GPHH:
                algorithm = new GPHHSchedulingAlgorithm();
                break;
            default:
                algorithm = new StaticSchedulingAlgorithm();
                break;
...
//                        + VmList.getById(getVmsCreatedList(), vmId).getHost().getId());
    ...
    protected void processCloudletUpdate(SimEvent ev) {
        ...
        // here for scheduled list we run
    ...
    public double calculateDelay(){
        ...
        for (Object o : getVmsCreatedList()){
            CondorVM vm = (CondorVM) o;
            System.out.println("vmCreated : "+vm.getId()+" status: "+vm.getState());
            if (vm.getState()==WorkflowSimTags.VM_STATUS_BUSY){
                //System.out.print("calculate how much this vm delays us: ");
                for (Cloudlet c : getCloudletSubmittedList()) {
                    if (c.getVmId()==vm.getId()){
                        System.out.println("processing cloudlet: "+c.getCloudletId());
                        System.out.println(c.getCloudletFinishedSoFar()+" = finished so far.");
                        double estExecution = c.getCloudletTotalLength() / vm.getMips();
                        System.out.println("Estimated executiom of this cloudlet: "+estExecution);
                        //delay += estExecution;
                    }
                }
            }
    ...
    protected void processCloudletReturn(SimEvent ev) {
        ...
        //System.out.println("Returned cloudlet: "+job.getCloudletId()+" from vm: "+vm.getId());
